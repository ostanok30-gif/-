...
# ========== НАСТРОЙКИ ==========
TOKEN = "8828821963:AAH9t8dmVBPfDUVuE1Qs8Y8k33hJ_hCR6BY"
...
# ========== КОНВЕРТАЦИЯ АУДИО (БЕЗ ffmpeg) ==========
def convert_ogg_to_wav(input_path, output_path):
    """Конвертация через ffmpeg с проверкой"""
    try:
        ...
        # Если ffmpeg нет - пробуем локальный
        try:
            local_ffmpeg = os.path.join(os.path.dirname(__file__), "ffmpeg")
            cmd = [local_ffmpeg, "-i", input_path, "-acodec", "pcm_s16le", "-ac", "1", "-ar", "16000", output_path, "-y", "-loglevel", "quiet"]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            return result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0
        except:
            logger.error("FFMPEG не найден нигде!")
            return False
