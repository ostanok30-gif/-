...
# ========== РАСШИФРОВКА (МНОГО МЕТОДОВ) ==========
def transcribe_method1(audio_bytes):
    """Метод 1: pydub -> wav -> speech_recognition"""
    try:
...
def transcribe_method2(audio_bytes):
    """Метод 2: напрямую через Google Speech API"""
    try:
...
def transcribe_method3(audio_bytes):
    """Метод 3: через subprocess ffmpeg"""
    try:
...
def transcribe_audio_bytes(audio_bytes):
    ...
    methods = [transcribe_method1, transcribe_method2, transcribe_method3]
